...
class DukcapilPortalController extends Controller
{
    public function dukcapil(Request $request)
    {
        ...
        try {
            ...
            $zip = new \ZipArchive();
            $zipFilename = 'Dukcapil-Portal_review.zip';
        ...
        try {
            ...
            foreach ($header as $index => $columnName) {
                ...
                switch (true) {
                    ...
                    case (preg_match('/username/i', $columnName)):
                        $columns['username'] = $index;
                        break;
                
                    case (preg_match('/nama/i', $columnName)):
                        $columns['nama'] = $index;
                        break;
                    ...
                    case (preg_match('/level\s?pengguna/i', $columnName)):
                        $columns['level_pengguna'] = $index;
                        break;
                    }        
            }
    
            if (!isset($columns['full_name']) && !isset($columns['nama'])) {
    ...
    private function compareEmployees($activeEmployees, $applicationUsers)
    {
        ...
        foreach ($userRows as $user) {
            ...
            if (isset($userCols['nama']) && is_array($user)) {
                $userName = $this->getNameFromRecord($user, $userCols['nama']);
            } else {
                continue; 
            }
            ...
            $results[] = [
                ...
                'Level Pengguna' => $user[$userCols['level_pengguna']] ?? '',
    ...
    private function generateCSV($data)
    {
        ...
        fputcsv($output, ['Username', 'Nama', 'Status', 'Level Pengguna', 'Status Review', 'Remarks']);
